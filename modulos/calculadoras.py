...
with tab_rapida:
    ...
    dinero_arriesgar_r = bal_r * (riesgo_pct_r / 100.0)
    lotaje_r = (
        dinero_arriesgar_r / (sl_pips_r * val_pip_r)
        if sl_pips_r > 0 and val_pip_r > 0
        else 0.0
    )
    ...
    with cr1:
        st.metric("Riesgo Total ($)", f"${dinero_arriesgar_r:.2f}")
    with cr2:
        st.metric("Lotaje Recomendado", f"{lotaje_r:.2f}")
