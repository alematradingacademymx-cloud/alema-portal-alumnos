...
def render_biblioteca_guias():
    ...
    if os.path.exists(archivo_pdf):
        ...
        st.caption(
            "💡 **Tip de lectura:** En dispositivos móviles, gira la pantalla"
            " horizontalmente para ampliar el visor."
        )
    else:
        st.warning(
            f"⚠️ El archivo `{archivo_pdf}` no se encuentra en el repositorio."
        )


# Alias de compatibilidad
render_biblioteca = render_biblioteca_guias
